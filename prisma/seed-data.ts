 ...
 * Seed categories.
 *
 * Fictional demo markets, grouped by publication window. Nothing here refers to
 * a real market, operator or product. Names and descriptions are stored once
 * per market as `{ en, hi, or }` and resolved at the boundary — the row itself
 * is never duplicated per language.
 *
 * Deliberately free of `@/` path aliases: `prisma db seed` runs this file
 * through tsx, outside the Next.js module graph.
 */
...
/** Days of history the seed generates, including today. */
export const SEED_ARCHIVE_DAYS = 60;

/** Express Results was paused this many days ago; no entries after that. */
export const SEED_PAUSED_SINCE_DAYS_AGO = 11;
