...
/**
 * Rendered per request. The category set now lives in PostgreSQL and an
 * administrator can add one at any time, so there is no fixed slug list to
 * pre-generate — a market created a minute ago has to resolve immediately.
 *
...
export async function generateMetadata({
  ...
  // response starts streaming, so the 404 is a real status code rather than a
...
export default async function CategoryDetailPage({
  ...
  const [page, stats, range] = await Promise.all([
    listResults(query),
    getStatistics({ category: slug, days: 30 }, locale),
    getArchiveRange(),
  ]);
